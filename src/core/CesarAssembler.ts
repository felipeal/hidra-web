...
import { assert, assertUnreachable, removeItem } from "./utils/FunctionUtils";
...
export class CesarAssembler extends Assembler {
  ...
  protected buildInstruction(instruction: Instruction, args: string): void {
    ...
    if (instruction.hasParameter("a0")) {
      ...
      const { addressingModeCode, registerName, additionalWord } = this.extractArgumentParts(argumentList[parameterPos], { instruction, args });
  ...
  protected extractArgumentParts(argument: string, { skipValues, instruction, args }: { skipValues?: boolean, instruction?: Instruction, args?: string }): {
    ...
    for (const addressingMode of this.machine.getAddressingModes()) {
      ...
      if (matchingGroups) {
        return {
          ...
          registerName: matchingGroups.register,
          additionalWord: matchingGroups.offset ? (skipValues ? 0 : this.parseAdditionalWord(matchingGroups, { instruction, args })) : null
        };
      }
    }
  ...
  protected parseAdditionalWord(matchingGroups: Record<string, string>, { instruction, args }: { instruction?: Instruction, args?: string }): number {
    assert(instruction && args, "Instruction and arguments must be provided when not skipping value calculations.");

    let argument = matchingGroups.offset;

    if (matchingGroups.register.toUpperCase() === "R7") { // Indexed R7 uses relative label offsets
      argument = this.labelToRelativeValue(matchingGroups.offset, { instruction, args });
    }

    return this.argumentToValue(argument, { isImmediate: false, allowLabelOffset: false });
  }

  protected labelToRelativeValue(argument: string, { instruction, args }: { instruction: Instruction, args: string }): string {
    if (this.labelPCMap.has(argument.toLowerCase())) {
      const targetAddress = this.labelPCMap.get(argument.toLowerCase())!;
      const nextPCValue = this.machine.toValidAddress(this.pcValue + this.calculateNumBytes(instruction, args));
